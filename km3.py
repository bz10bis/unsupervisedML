...
class KMeans(object):
    ...
    def calc_distance(self, point, centroid):
        return np.linalg.norm(point.features - centroid.features)

    def calc_distance_2(self, point, centroid):
        dist_sum = 0
    ...
    def update_centroids_2(self):
        for c in self.centroid:
            points = [x.features for x in self.X if x.label == c.label]
            c.features = np.mean(points)
